...
| 13 | 卡带**加载成功**,但 `Instantiate the wasm module` 后报 `allocate linear memory failed` | **分配器 ABI 错位**:手拼构建时设了 `WASM_MEM_ALLOC_WITH_USAGE=1`,该宏把 malloc 回调签名变成 `(usage, size)` 两参,而 ESP-IDF 平台层 `os_malloc(unsigned)` 只认一参 → 第一个参数(枚举值 1)被当成 size,只分配 1 字节却要装 64KB | 改用**官方 ESP-IDF 集成**(不设该宏 → 默认 0,签名一致);作者 WAMR pin=`21330990…`(与 c4aa1ded 差异仅 shared-heap,无 allocator 补丁)。**症状("内存不够")与根因(ABI)离得很远,只能读源码发现** | ✅ |

| 15 | `on` 日志正常但灯不亮 / 乱闪 / `readback` 与预期不符 | 三种成因:① 引脚已被外设占用(IO1/IO2 = LCD 的 SCLK/DC,`gpio_set_level` 无效);② **接错引脚**(灯压根不在这根线上);③ 被负载钳位(无电阻 LED 把 3.3V 拉塌到 ~2V,低于 2.5V 判定门限) | 命令内**回读真实电平**并打印(`readback=`);接线前用万用表/探针命令确认孔位;限流电阻不可省 | ✅ |

| 16 | 输出脚的 `gpio_get_level()` **恒为 0**(回读永远失败) | `gpio_config()` 对**纯 `GPIO_MODE_OUTPUT`** 会调用 `gpio_input_disable()`——输入缓冲被关闭,回读读的是关闭的输入通道 | 需要回读的脚用 **`GPIO_MODE_INPUT_OUTPUT`**(输出与输入缓冲都打开);纯输出脚不必回读 | ✅ |
...
| C12 | "数据"和"代码"都在固件里,区别在 **PC 会不会跳进去执行** | `nm` 看段:卡带 `__game_card` = `D`(数据段),解释器 `wasm_interp_call_func_bytecode` = `t`(代码段)。C 编译器对卡带只做"打包"(字面量→字节),不做"翻译"(wasm→机器码)。wasm 是"另一台虚构机器的机器码",CPU 只执行"模拟那台机器的程序"(WAMR) |
| C13 | "运行时链接"不是链接器 | 其实是 WAMR 加载卡带时用 `strcmp` 在 `native_symbols[]` 里**按名字查函数指针**(查电话簿):`{ "blit", wrap_w4_runtimeBlit, "(*iiiii)" }`。一次解析、后续走指针,板子上没有编译器/链接器 |

## ⚠️ 复发条件(什么情况下老坑会再犯)

| 老坑 | 再犯的触发条件 |
|---|---|
| #13 分配器 ABI | **又有人手动打开 `WASM_MEM_ALLOC_WITH_USAGE=1`** → 必须同时提供 `(usage, size)` 签名的分配器包装,否则枚举值又会被当 size |
| #13 及各类编译期宏坑 | **再次手拼 WAMR 源文件列表/宏**(不用官方集成)→ 宏与平台层的配套关系失去上游保证 |
| C11 menuconfig 4 杀 | 每次保存 menuconfig 后不检查 4 项(Console/分区/Flash/SPIRAM) |
| #3 build 残留 | 复制工程后不删 `build/` |
| #6 SPIRAM | 换板子不看 sdkconfig |

## 待补(推图完成后)

- [ ] 饿死任务实验(优先级 7 + 空转 vs vTaskDelay)
- [ ] 每个章节的"翻车记录"(推到哪记到哪)
- [ ] **优化**:卡带数组加 `const` → 从 `.data`(RAM,占 20KB)移到 `.rodata`(flash);需处理 `wasm_runtime_load()` 的非 const 形参(强转 + 确认加载器不写该内存)
- [ ] **清理**:删除 `components/wamr/wamr_extra.c`(早期自造的 invokeNative 桩,已不参与编译)
